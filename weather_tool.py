def generate_weather_report(zip_code: str, reference_temp: float) -> str:
    import pandas as pd
    ...
    # reference_temp is passed as parameter
    # facility_type removed as unused
    if use_zip:
    ...
    summary['pct_above_ref'] = summary['year'].map(lambda y: (summary.loc[summary['year'] == y, 'above_ref'].values[0] / total_hours_per_year[y]) * 100 if y in total_hours_per_year else 0)
